...
const greeting = `👋 Добро пожаловать в VITAL — пространство здоровья, энергии и омоложения 🌿

Здесь собраны лучшие решения для восстановления организма, повышения тонуса и продления молодости.
Мы объединяем современные технологии, натуральные продукты и осознанный подход к жизни.
Вы можете выбрать товары для очищения, восстановления и поддержки энергии каждый день.
Следите за новинками, акциями и рекомендациями для вашего здоровья.
Начните с малого — и почувствуйте изменения уже в ближайшее время.`;

/** Текст под фото приветствия */
const WELCOME_PHOTO_CAPTION = `👋 Добро пожаловать в VITAL — пространство здоровья, энергии и омоложения 🌿
...
/** Сообщение через 30 сек после приветствия: про бесплатную продукцию и реф. ссылку */
const WELCOME_FOLLOW_UP_TEXT = `🎁 Вы сможете получать продукцию бесплатно — просто делитесь ссылкой с друзьями, накапливайте бонусы и заказывайте товары в нашем магазине.`;

/** Отправить через 30 сек второе сообщение с текстом про бонусы и реф. ссылкой */
function scheduleWelcomeFollowUp(ctx: Context) {
  ...
  setTimeout(async () => {
    try {
      ...
      if (!user) return;
      const profile = user.partner ?? await getOrCreatePartnerProfile(user.id, 'DIRECT');
      ...
      await telegram.sendMessage(chatId, `${WELCOME_FOLLOW_UP_TEXT}\n\n🔗 Ваша реферальная ссылка:\n${botLink}`);
    } catch (err: any) {
      console.warn('⚠️ Welcome follow-up failed:', err?.message || err);
    }
  }, 30000);
}
