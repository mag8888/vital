    ...
    async register(bot) {
        bot.command('add_balance', async (ctx) => {
            try {
                ...
                await ctx.reply('💰 <b>Пополнение баланса</b>\n\n' +
                    ...
                    'Если передумали, напишите «отмена».', { parse_mode: 'HTML' });
            }
            catch (error) {
        ...
        bot.on('text', async (ctx, next) => {
            ...
            if (!Number.isFinite(amount) || amount <= 0) {
                await ctx.reply('⚠️ Введите корректную сумму числом, например <code>500</code> или <code>799.50</code>.\n' +
                    'Чтобы отменить операцию, напишите «отмена».', { parse_mode: 'HTML' });
                return;
            }
            if (amount < 10) {
