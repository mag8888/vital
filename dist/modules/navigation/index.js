...
async function sendWelcomeWithPhoto(ctx, options) {
    ...
    try {
        await ctx.replyWithPhoto({ url: getWelcomePhotoUrl() }, { caption, parse_mode: 'HTML', reply_markup: replyMarkup });
    }
    catch (err) {
    ...
    async register(bot) {
        ...
        bot.hears(['🎶 Звуковые матрицы Гаряева', 'Звуковые матрицы'], async (ctx) => {
            ...
            try {
                ...
                console.warn('hears gift_audio failed:', e?.message);
        ...
        bot.action('nav:gift_audio', async (ctx) => {
            ...
            try {
                ...
                console.warn('nav:gift_audio failed:', e?.message);
                await ctx.reply('🎵 Аудиофайлы загружаются. Попробуйте позже или напишите в поддержку.');
            }
        });
