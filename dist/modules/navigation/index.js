...
<b>Plazma Water</b> — это инновационный водный раствор, содержащий микроэлементы в уникальной плазменной наноструктуре. Благодаря особой технологии, частицы в составе имеют нано размер и равномерно распределены в воде, что обеспечивает их естественное взаимодействие с биологическими системами организма.
...
<b>⚠️ Важно:</b> Plazma Water не является лекарственным средством и не предназначен для лечения или диагностики заболеваний. Его использование направлено на поддержание оптимального водно-минерального баланса, повышение комфорта, энергии и общего самочувствия.
...
async function handleSupportMessage(ctx) {
    ...
    if (bot) {
        ...
        try {
            ...
            // Confirm to user
            await ctx.reply('✅ Ваше сообщение принято.\n\nПожалуйста, свяжитесь с нашим менеджером для консультации (аккаунт @Aurelia_8888).');
        }
        catch (error) {
...
async function sendWelcomeVideo(ctx) {
    ...
    const webappUrl = env.webappUrl || 'https://plazma-production.up.railway.app/webapp';
        ...
        reply_markup: {
            inline_keyboard: [
                [
                    {
                        text: '🎥 Смотреть видео',
                        web_app: { url: `${webappUrl}?video=${encodeURIComponent(WELCOME_VIDEO_URL)}` },
                    },
                ],
                [
                    {
