...
export class UserControllerImpl extends BaseControllerClass {
    constructor() {
        super();
    ...
    async detailed(req, res, next) {
        try {
            ...
            const users = await this.userService.getUsersWithStats({}, { field: sort, direction: order }, { page: 1, limit: 100 });
    ...
    async getPartners(req, res, next) {
        try {
            ...
            const partners = await this.userService.getReferralChain(userId, parseInt(level));
    ...
    async changeInviter(req, res, next) {
        try {
            const { id } = req.params;
    ...
    generateUsersDetailedHTML(users) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
