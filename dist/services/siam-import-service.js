 ...
 */
function convertTHBToPZ(thbPrice) {
...
async function downloadAndUploadImage(imageUrl, productId) {
    ...
    const alternativeUrls = [imageUrl];
...
async function importProduct(product) {
    ...
    if (aiEnabled) {
        ...
        translatedSummary = await aiTranslationService.translateSummary(product.englishSummary, translatedTitle);
        console.log('  🔄 Перевод полного описания...');
        translatedDescription = await aiTranslationService.translateProductDescription(product.englishDescription, 'cosmetic', {
            preserveStyle: true,
 ...
 */
export async function importSiamProducts() {
 ...
 */
async function extractImageFromProductPage(slug) {
    try {
...
export async function updateProductImages() {
    ...
    const productMapByKeyword = new Map();
    for (const siamProduct of siamProducts) {
        ...
        for (const [ruKeyword, enKeyword] of keywordMap.entries()) {
            if (titleLower.includes(enKeyword)) {
                ...
                productMapByKeyword.get(ruKeyword).push(siamProduct);
            }
        }
    }
    for (const product of allProducts) {
        try {
            let siamProduct = null;
            ...
            if (!siamProduct) {
                for (const siamProd of siamProducts) {
                    if (!siamProd.englishTitle || !siamProd.imageUrl)
                        continue;
 ...
 */
export async function uploadAllProductImagesFromPages() {
