 ...
 */
import { escapeAttr, escapeHtml, formatDate } from './products-helpers.js';
 ...
 */
function generateFilters(categories, totalProducts) {
    let html = `
    <div class="filters">
 ...
 */
function generateProductActions(product) {
    return `
    <div class="product-actions">
      <button 
        ...
        data-instruction="${escapeAttr(product.instruction)}"
      ...
      <button class="instruction-btn" data-instruction-id="${escapeAttr(product.id)}" data-instruction-text="${escapeAttr(product.instruction)}" onclick="showInstructionSafe(this)" style="background: #28a745;">📋 Инструкция</button>
 ...
 */
function generateModals(categories) {
    return `
 ...
 */
function generateAddSubcategoryModal(categories) {
    const categoryOptions = categories.map(cat => `<option value="${escapeAttr(cat.id)}">${escapeHtml(cat.name)}</option>`).join('');
    return `
