...
function renderAdminShellStart(opts) {
    const { title, activePath, buildMarker } = opts;
    const isActive = (href, opts) => {
        const also = opts?.also || [];
        const prefixes = opts?.prefixes || [];
        if ([href, ...also].includes(activePath))
            return 'active';
        if (prefixes.some((p) => activePath.startsWith(p)))
            return 'active';
        return '';
    };
    return `
...
router.post('/api/categories/:id/update', requireAdmin, async (req, res) => {
    try {
        ...
        if (!id)
            return res.status(400).json({ success: false, error: 'category_id_required' });
        if (!name)
            return res.status(400).json({ success: false, error: 'Название категории обязательно' });
        const slug = name.toLowerCase().replace(/\s+/g, '-');
        const data = { name, slug, description, imageUrl: imageUrl || null };
        if (typeof isVisibleRaw === 'boolean')
            data.isVisibleInWebapp = isVisibleRaw;
        if (String(isVisibleRaw) === 'true' || String(isVisibleRaw) === 'false')
            data.isVisibleInWebapp = (String(isVisibleRaw) === 'true');
...
router.post('/categories/:id/toggle-active', requireAdmin, async (req, res) => {
    try {
        const id = String(req.params.id || '').trim();
        if (!id)
            return res.redirect('/admin/categories?error=category_not_found');
...
router.get('/categories', requireAdmin, async (req, res) => {
    ...
    `;
        ...
        <tr>
          ...
          <td><span class="pill">${Number(cat.productsCount || 0)}</span></td>
          <td>
          ...
          <td>${cat.isVisibleInWebapp === false ? 'Нет' : 'Да'}</td>
    ...
    `;
      ...
      `;
          ...
          <div class="product-card"
            ...
            ${product.sku ? `<div style="margin: 4px 0; font-size: 12px; color: #6b7280;"><strong>ID товара (Item):</strong> <span style="color: #1f2937; font-weight: 600;">${escapeHtml(product.sku)}</span></div>` : ''}
            ...
            <div class="product-actions">
              <button 
                ...
                data-instruction="${escapeAttr((product.instruction || '').substring(0, 5000))}"
        ...
        <div id="productsTableContainer" style="display:none; margin-top: 14px;">
          <div style="overflow:auto; background:#fff; border-radius:12px; box-shadow: 0 6px 16px rgba(0,0,0,0.08); border:1px solid #e5e7eb;">
            ...
            return ('<tr ' +
                ...
                'data-instruction="' + escapeAttr((p.instruction || '').substring(0, 5000)) + '" ' +
                'data-price="' + p.price + '" ' +
                'data-category-id="' + escapeAttr(p.categoryId) + '" ' +
                'data-active="' + (p.isActive ? 'true' : 'false') + '" ' +
                'data-russia="' + (p.availableInRussia ? 'true' : 'false') + '" ' +
                'data-bali="' + (p.availableInBali ? 'true' : 'false') + '" ' +
...
router.post('/api/products/translate', requireAdmin, async (req, res) => {
    try {
        const { text, type, productName, productType } = req.body;
        if (!text || !text.trim()) {
            return res.status(400).json({
                success: false,
        ...
        try {
            if (type === 'title') {
                ...
                // description
                translatedText = await aiTranslationService.translateProductDescription(text, productType || 'cosmetic', {
                    preserveStyle: true,
...
router.post('/api/certificate-types', requireAdmin, upload.single('image'), async (req, res) => {
    try {
        ...
        if (priceRub <= 0 || valueRub <= 0)
            return res.status(400).json({ error: 'Цена и номинал должны быть больше 0' });
        let imageUrl = null;
        if (req.file) {
            if (!isCloudinaryConfigured()) {
...
router.put('/api/certificate-types/:id', requireAdmin, upload.single('image'), async (req, res) => {
    try {
        ...
        if (req.body?.priceRub !== undefined)
            data.priceRub = Number(req.body.priceRub || 0) || 0;
        if (req.body?.valueRub !== undefined)
            data.valueRub = Number(req.body.valueRub || 0) || 0;
        if (req.body?.sortOrder !== undefined)
            data.sortOrder = Number(req.body.sortOrder || 0) || 0;
        if (req.body?.description !== undefined)
            data.description = String(req.body.description || '').trim() || null;
        if (req.body?.isActive !== undefined)
            data.isActive = String(req.body.isActive) === '1' || String(req.body.isActive) === 'true';
        if (req.file) {
            if (!isCloudinaryConfigured()) {
                return res.status(400).json({ error: 'Cloudinary не настроен — загрузка обложки недоступна' });
            }
        ...
        const updated = await prisma.certificateType.update({ where: { id }, data });
...
router.post('/api/certificates/issue', requireAdmin, async (req, res) => {
    try {
        ...
        if (valueRub <= 0)
            return res.status(400).json({ error: 'Номинал должен быть больше 0' });
        let userId = null;
        if (telegramId) {
        ...
        for (let attempt = 0; attempt < 6; attempt++) {
            const code = genCode();
            try {
                created = await prisma.giftCertificate.create({
        ...
        if (!created)
            return res.status(500).json({ error: 'Не удалось сгенерировать код' });
...
router.post('/chats/:telegramId/reply', requireAdmin, express.urlencoded({ extended: true }), async (req, res) => {
    try {
        ...
        try {
            ...
            const bot = await getBotInstance();
            if (bot) {
                const escapeTelegramHtml = (s) => s
                    .replace(/&/g, '&amp;')
                    .replace(/</g, '&lt;')
                    .replace(/>/g, '&gt;');
                await bot.telegram.sendMessage(telegramId, `💬 <b>Ответ службы поддержки:</b>\n\n${escapeTelegramHtml(text)}`, { parse_mode: 'HTML' });
            }
        }
        catch (sendErr) {
...
router.post('/api/balance-topups/:id/approve', requireAdmin, async (req, res) => {
    try {
        ...
        const request = await prisma.balanceTopUpRequest.findUnique({ where: { id } });
        if (!request)
...
router.get('/balance-topups', requireAdmin, async (_req, res) => {
    try {
        ...
        const requests = await prisma.balanceTopUpRequest.findMany({
            orderBy: { createdAt: 'desc' },
            include: { user: true }
        });
        const escapeHtml = (str) => String(str ?? '')
...
router.post('/api/specialists', requireAdmin, async (req, res) => {
    try {
        ...
        if (!categoryId)
            return res.status(400).json({ success: false, error: 'categoryId обязателен' });
        if (!specialtyId)
            return res.status(400).json({ success: false, error: 'specialtyId обязателен' });
        const specialty = await prisma.specialistSpecialty.findUnique({ where: { id: String(specialtyId) } });
        if (!specialty)
            return res.status(400).json({ success: false, error: 'Специальность не найдена' });
