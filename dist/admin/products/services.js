 ...
 * Создать товар
 */
export async function createProduct(data, imageBuffer) {
 ...
 * Обновить товар
 */
export async function updateProduct(productId, data, imageBuffer) {
    ...
    if (data.instruction !== undefined)
        updateData.instruction = data.instruction?.trim() || null;
    if (data.categoryId)
    ...
    if (data.availableInRussia !== undefined)
        updateData.availableInRussia = data.availableInRussia;
    if (data.availableInBali !== undefined)
        updateData.availableInBali = data.availableInBali;
    if (imageUrl)
 ...
 */
export async function saveProductInstruction(productId, instruction) {
    const product = await prisma.product.findUnique({
        where: { id: productId }
    });
    if (!product) {
