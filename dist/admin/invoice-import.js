...
function parseCsvBuffer(buf) {
    ...
    if (lines.length < 2)
        return { rows: [], errors: ['CSV пустой или содержит только заголовок'] };
    ...
    const errors = [];
    if (idxProductId < 0)
        errors.push('Нет колонки productId');
    if (idxRate < 0)
        errors.push('Нет колонки invoiceRateTHB');
    if (idxQty < 0)
        errors.push('Нет колонки invoiceQty');
    if (errors.length)
        return { rows: [], errors };
    const rows = [];
    ...
    try {
        ...
        for (const r of actionable) {
            if (seen.has(r.productId))
                errors.push(`Дубликат productId в CSV: ${r.productId}`);
...
router.get('/api/test-price-calculation', requireAdmin, async (req, res) => {
    try {
        ...
        const purchasePriceBAT = parseFloat(purchasePrice);
