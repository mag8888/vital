...
import { getActiveCategories, getProductsByCategory, getAllActiveProducts } from '../services/shop-service.js';
    ...
    try {
        ...
        try {
            ...
            if (bot) {
                ...
                const adminMessage = '📨 <b>Сообщение в поддержку (WebApp)</b>\n\n' +
...
async function getOrCreateWebappUser(req) {
    const telegramUser = getTelegramUser(req);
    if (!telegramUser)
        return null;
    const { prisma } = await import('../lib/prisma.js');
...
async function getFavoritesSetForUserId(userId) {
    const { prisma } = await import('../lib/prisma.js');
...
router.post('/api/orders/create', async (req, res) => {
    try {
        ...
        const totalPz = orderItemsForTotal.reduce((sum, it) => {
        ...
        let certAppliedPz = 0;
        let certRemainingPz = null;
        let certCodeUsed = null;
        if (certificateCode && String(certificateCode).trim()) {
            const code = String(certificateCode).trim().toUpperCase();
            const cert = await prisma.giftCertificate.findUnique({ where: { code } }).catch(() => null);
            if (!cert) {
...
router.get('/api/certificates/my', async (req, res) => {
    try {
        ...
        const certs = await prisma.giftCertificate.findMany({
            where: { userId: user.id, status: 'ACTIVE', remainingPz: { gt: 0 } },
...
router.post('/api/certificates/buy', async (req, res) => {
    try {
        ...
        if (pricePz <= 0 || valuePz <= 0)
            return res.status(400).json({ error: 'Некорректные параметры сертификата' });
        const totalCostPz = pricePz * qty;
        const currentBalance = Number(user.balance || 0) || 0;
        ...
        res.json({
            ...
            certificates: created.map((c) => ({ id: c.id, code: c.code, remainingPz: c.remainingPz }))
        });
    }
    catch (error) {
...
router.post('/api/balance/topup-receipt', upload.single('receipt'), async (req, res) => {
    try {
        ...
        await prisma.balanceTopUpRequest.create({
            data: {
                userId: user.id,
...
router.post('/api/balance/topup', async (req, res) => {
    try {
        ...
        const userEmail = user.phone ? `${user.telegramId}@vital.temp` : `user_${user.telegramId}@vital.temp`;
...
router.get('/api/delivery/methods', async (req, res) => {
    try {
        ...
        if (city.length < 2)
            return res.json({ success: true, city, methods: [] });
        const { prisma } = await import('../lib/prisma.js');
        ...
        const methods = [];
        if (pickupEnabled)
            methods.push({ id: 'pickup', title: 'До пункта выдачи', priceRub: pickupPriceRub });
        if (courierEnabled)
            methods.push({ id: 'courier', title: 'Курьером до двери', priceRub: courierPriceRub });
        const warning = provider === 'cdek' && (!cdekClientId || !cdekClientSecret)
...
router.get('/api/plazma/products', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            products: products.slice(0, parseInt(limit) || 20)
        });
    }
    catch (error) {
    ...
    try {
        ...
        try {
            ...
            if (bot && user) {
                ...
                contactInfo += `🆔 User ID: ${user.id}\n`;
                contactInfo += `🆔 Telegram ID: ${telegramUser.id}`;
                const orderMessage = '🛍️ <b>Новый заказ Plazma от пользователя</b>\n\n' +
                    `👤 <b>Пользователь:</b> ${user.firstName || ''} ${user.lastName || ''}\n` +
                    `${contactInfo}\n\n` +
