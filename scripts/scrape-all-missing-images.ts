  ...
  for (const siteProduct of allProductsFromSite) {
    try {
      ...
      if (dbProduct.imageUrl && dbProduct.imageUrl.trim() !== '') {
        console.log(`   ✓ Изображение уже есть`);
        skipped++;
        continue;
      }
      ...
      // Если изображения нет в списке, пробуем извлечь со страницы продукта
      if (!imageUrl) {
        console.log(`   🔍 Извлекаю изображение со страницы продукта...`);
